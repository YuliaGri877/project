...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (servicesSection) {
        ...
        for (const serviceKey in servicesData) {
            if (servicesData.hasOwnProperty(serviceKey)) {
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (headerMenu) {
        ...
        const createLink = (item) => {
            const activeClass = item.active ? 'header__item-link--active' : '';
            return `
                <li class="header__item">
                    <a href="${item.link}" class="header__item-link ${activeClass}">${item.title}</a>
                </li>
            `;
        };

        // Очищаем существующее меню перед добавлением новых элементов
        headerList.innerHTML = '';

        // Добавляем элементы меню
        menuData.forEach(item => {
            const linkElement = createLink(item);
            headerList.insertAdjacentHTML('beforeend', linkElement);
        });

        // Добавляем кнопку "Заказать звонок"
        const consultationButton = `
            <button class="header__consultation-button button">Заказать звонок</button>
        `;
        headerList.insertAdjacentHTML('beforeend', consultationButton);
    }
});